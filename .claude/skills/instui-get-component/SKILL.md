...
description: >
  ...
  SideNavBar". Also invoke proactively before writing code that uses a component for the
  first time in a session — check actual prop types before writing, not after TypeScript
  catches a mistake. Reads the actual TypeScript prop types from the installed package.
---
