---
name: instui-get-tokens
description: >
  Dynamically explores available token keys and values for any layer of the InstUI v11.7.2
  theme system: sharedTokens, semantics, components, or primitives. Invoke for discovery
  of tokens NOT already covered by instui-reference-tokens — component-specific themeOverride keys
  ("what tokens can I override on Button"), obscure semantics paths ("what does
  semantics.color.stroke contain"), accent token names, or any token outside the
  sharedTokens reference table in instui-reference-tokens. Do not invoke for sharedTokens.background,
  sharedTokens.stroke, sharedTokens.borderRadius, sharedTokens.spacing, or other groups
  already documented inline in instui-reference-tokens — read that skill instead. Do not invoke for
  questions about how to use tokens in code — use instui-reference-tokens for that.
---
